...
impl OmniPaxosServer {
    ...
    pub async fn new(config: OmniPaxosKVConfig) -> Self {
        ...
        let storage_path = format!("storage_node_{}", config.local.server_id);

        // RocksDB options
        let mut state_store_opts = rocksdb::Options::default();
        state_store_opts.create_if_missing(true);
        state_store_opts.create_missing_column_families(true);

        let log_store_opts = rocksdb::Options::default();

        // Persistent storage configuration
        let mut storage_config = PersistentStorageConfig::default();
        storage_config.set_path(storage_path);
        storage_config.set_database_options(state_store_opts);
        storage_config.set_log_options(log_store_opts);

        let storage = PersistentStorage::with(storage_config).unwrap();

        let omnipaxos_config: OmniPaxosConfig = config.clone().into();

        let omnipaxos_msg_buffer =
            Vec::with_capacity(omnipaxos_config.server_config.buffer_size);
    ...
    fn send_outgoing_msgs(&mut self) {
        ...
        for msg in self.omnipaxos_msg_buffer.drain(..) {
            ...
            let cluster_msg =
                ClusterMessage::OmniPaxosMessage(msg);
    ...
    fn save_output(&mut self) -> Result<(), std::io::Error> {
        ...
        let mut output_file =
            File::create(&self.config.local.output_filepath)?;
